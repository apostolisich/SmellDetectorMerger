package smelldetectormerger.preferences;

public class PreferenceConstants {

	public static final String USE_ALL_DETECTORS = "useAllDetectors";
	public static final String CHECKSTYLE_ENABLED = "checkStyleEnabled";
	public static final String DUDE_ENABLED = "dudeEnabled";
	public static final String JDEODORANT_ENABLED = "jDeodorantEnabled";
	public static final String JSPIRIT_ENABLED = "jSpIRITEnabled";
	public static final String PMD_ENABLED = "pmdEnabled";
	
}
